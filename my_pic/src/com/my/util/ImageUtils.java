...
public class ImageUtils {
	...
	private static final String ImagePath = "/sdcard/Android/data/com.wy.shb/";

	public static Bitmap loadImage(Activity activity, ImageView imageView,
			Uri uri) {
		if (Tools.isDebug)
	...
	private static Bitmap compress(Bitmap bitmap, int target_size) {
		...
		if (Tools.isDebug)
			Log.i(TAG, "compress: length = " + length + " kb, target_size = "
					+ target_size + " kb");
	...
	public static String saveImage(Bitmap bitmap, int quality) {
		...
		try {
			...
			if (bitmap.compress(Bitmap.CompressFormat.JPEG, quality, out)) {
				Log.d(TAG, "saveImage seccess: fileName= " + filePath
						+ ", quality = " + quality);
				out.flush();
				out.close();
				path = filePath;
	...
	/**
	 * convert view to integral bitmap and return it 获取view的完整视图图片（即使没有显示出来的部分）
	 * 
	 * @param view
	 *            : view or layout
	 * @return
	...
	// 截屏后返回路径
	public static String shareShotScreen(View view) {
		Bitmap bitmap = convertViewToBitmap(view, view.getWidth(),
				view.getHeight());
		String path = saveImage25(bitmap);
		return path;
	}
}
